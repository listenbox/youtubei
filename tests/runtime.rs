...
async fn basic_info_uses_upstream_client_and_returns_live_format_handles() -> youtubei::Result<()> {
    ...
    let (fetch, requests) = fixture_fetch(&engine, vec![json!({
        ...
        "streamingData": {"expiresInSeconds": "3600", "formats": [{
            "itag": 18, "url": "https://example.googlevideo.com/video",
            "mimeType": "video/mp4; codecs=\"avc1.42001E, mp4a.40.2\"",
            "bitrate": 100000, "width": 640, "height": 360, "qualityLabel": "360p", "audioQuality": "AUDIO_QUALITY_MEDIUM"
        }], "adaptiveFormats": [audio_format()]}
    })]).await?;
    let yt = Innertube::create_in(
        &engine,
        SessionOptions {
    ...
    let formats = info.formats().await?;
    assert_eq!(formats.len(), 1);
    assert_eq!(formats[0].info().height, Some(360.0));
    assert!(formats[0].info().has_video);
    assert!(formats[0].info().has_audio);
    assert_eq!(
        formats[0].decipher(None).await?,
        "https://example.googlevideo.com/video"
    );
    assert_eq!(
